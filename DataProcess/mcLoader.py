...
class McDataLoader(Dataset):
    def __init__(self,
        ...
        # 记录40个类别名称的文件
        self.catfile = os.path.join(self.root, f"mcgill_names.txt")
        self.cat = [line.strip() for line in open(self.catfile)]
        self.classes = dict(zip(self.cat, range(len(self.cat))))
        # print(f'self.classes:{self.classes}')
        print('-----McGill Dataset!-----')
        self.nviews = 1
        self.r = rotation

        shape_ids = {}
        shape_ids["test"] = [line.strip() for line in open(os.path.join(self.root, f"mcgill_names.txt"))]
...
#     DATA_PATH = 'E:/Project/Others/mcgill_op_npy'
#     test_dataset = McDataLoader(root=DATA_PATH, npoint=1024, gen_number=0)
#     test_dataloader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=8, shuffle=True, num_workers=0)
#     for batch_id, data in enumerate(test_dataloader):
#         print(data.keys())
#         print(data['pos'].shape)  # 8*3*1024
